...
void setup() {
  ...
  Serial.println("Sensors warming up. Please wait 30 seconds...");
  Serial.println("Ensure sensors are in clean air during this process.");
  ...
  // --- Calibration Phase for MQ-135 and MQ-2 ---
  Serial.println("Calibrating MQ-135 and MQ-2 (20 seconds)...");
  const int lib_calibration_samples = 20;
  float r0_mq135_sum = 0;
  float r0_mq2_sum = 0;
  for (int i = 1; i <= lib_calibration_samples; i++) {
    mq135.update();
    mq2.update();
    r0_mq135_sum += mq135.calibrate(RatioMQ135CleanAir);
    r0_mq2_sum += mq2.calibrate(RatioMQ2CleanAir);
    Serial.print(".");
    delay(1000); // 1-second delay between samples
  }
  float final_r0_mq135 = r0_mq135_sum / lib_calibration_samples;
  float final_r0_mq2 = r0_mq2_sum / lib_calibration_samples;
  mq135.setR0(final_r0_mq135);
  mq2.setR0(final_r0_mq2);
  Serial.println("\nMQ-135 and MQ-2 calibration complete.");

  // --- Calibration Phase for Custom Smoke Sensor ---
  Serial.println("Calibrating custom Smoke Sensor (this takes ~25 seconds)...");
  // Call your custom calibration function ONCE. It has its own internal loop.
  Ro2 = MQCalibration(MQ_PIN);
  Serial.println("Custom Smoke Sensor calibration complete.");

  // --- Print Final Results ---
  Serial.println("\n--- Calibration Results ---");
  Serial.print("Final MQ-135 R0: "); Serial.println(final_r0_mq135);
  Serial.print("Final MQ-2 R0:   "); Serial.println(final_r0_mq2);
  Serial.print("Final Smoke R0:  "); Serial.println(Ro2);
  Serial.println("---------------------------------");
  
  // Check for sensor issues
  if (isinf(final_r0_mq135) || final_r0_mq135 == 0 || isinf(final_r0_mq2) || final_r0_mq2 == 0) {
    Serial.println("Warning: MQ-135/MQ-2 connection issue detected.");
  }
  if (isinf(Ro2) || Ro2 == 0) {
    Serial.println("Warning: Custom Smoke Sensor connection issue detected.");
  }
